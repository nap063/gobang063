...
import wuziqi2018.GameTree5.Nodes;

public class GameTree6 {
	private int MAX=0;
	private int MIN=0;
	private int depth=5;											//默认博弈深度
	private ChessBoard cB;
	private Vector<Integer> result=null;							//最优解
	public Vector<Integer> getRut(){
		return result;
	}
	public GameTree6(ChessBoard cBoard) {
	...
	public GameTree6(ChessBoard cBoard ,int deep) {					//可修改默认博弈深度
		this(cBoard);
		depth=deep;
	}
...
//			System.out.print("当前Nod数"+Nnum+"深度:"+deep+"Max:"+Max);
				...
				for(;i2<15;i2++){
					...
					for(;j<15;j++){
						if (gPos[i2][j]==0&&testPos1(gPos, i2, j)) {
							gPos[i2][j]=Max;
							Nodes sNodes;
							if(fiveEst(gPos, i2, j)){
								 sNodes=new Nodes(gPos, Min, 0);
								sNodes.setNodeAxis(i2,j);
								ndList.add(sNodes);	
								
								breakOut=1;
							}else if(fiveEst2(gPos, i2, j)){
								sNodes=new Nodes(gPos, Min, 0);
								sNodes.setNodeAxis(i2,j);
								ndList.add(sNodes);	
								...
								sNodes=new Nodes(gPos, Min, deep-1, weight,cutFlag);
								sNodes.setNodeAxis(i2, j);		
	...
	public boolean fiveEst2(int[][] pos,int x,int y){
		int miX,maX,miY,maY;
		int max5=(MIN==1?1:32);
		miX=(x<=4?0:x-4);
		maX=(x>=10?14:x+4);
